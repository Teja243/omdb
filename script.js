...
const getResult = async (e) => {
    const url = `https://www.omdbapi.com/?apikey=abf855e5&s=${input.value}&page=1`;
...
function addMovieToFavorites(movieData) {
    favMovies.push(movieData);
    localStorage.setItem('favMovies', JSON.stringify(favMovies));
    updateFavoriteList();
...
function deleteMovieFromFavorites(movieData) {
    const movieIndex = favMovies.indexOf(movieData);
    if (movieIndex !== -1) {
        favMovies.splice(movieIndex, 1);
        localStorage.setItem('favMovies', JSON.stringify(favMovies));
        updateFavoriteList();
        showToast(`${movieData.Title} removed from favorites!`);
    }
}
...
deleteButtons.forEach(deleteBtn => {
    deleteBtn.addEventListener('click', () => {
        const movieIndex = favMovies.indexOf(movieData);
        if (movieIndex !== -1) {
            deleteMovieFromFavorites(movieData);
        }
    });
});
